...
Training loop supporting:
  - R(2+1)D-18  (pretrained Kinetics-400) — strong baseline
  - SlowFast R50 (pretrained Kinetics-400) — best accuracy

Usage:
    python training/train.py                   # R(2+1)D (default)
    python training/train.py --model slowfast  # SlowFast
"""
...
from utils.dataset import get_dataloaders
from models.cnn3d import build_model, unfreeze_backbone
from models.slowfast import build_slowfast_model, unfreeze_slowfast, prepare_slowfast_input
...
BATCH_SIZE  = 8
NUM_WORKERS = 4     # 0 on Windows, 4 on Colab/Kaggle
...
def train_one_epoch(model, loader, criterion, optimizer, device, is_slowfast=False):
    model.train()
    running_loss, correct, total = 0.0, 0, 0
    ...
    for batch_idx, (clips, labels) in enumerate(loader):
        ...
        if is_slowfast:
            inputs  = [x.to(device) for x in prepare_slowfast_input(clips)]
...
@torch.no_grad()
def validate(model, loader, criterion, device, is_slowfast=False):
    model.eval()
    running_loss, correct, total = 0.0, 0, 0
...
def run_phase(phase, model, train_loader, val_loader, num_classes,
              device, writer, best_val_acc, epochs, lr,
              epoch_offset=0, is_slowfast=False, ckpt_name="best_model.pth"):
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_loss, train_acc = train_one_epoch(
            model, train_loader, criterion, optimizer, device, is_slowfast
        )
        val_loss, val_acc = validate(
            model, val_loader, criterion, device, is_slowfast
        )
        scheduler.step()
...
def train_r2plus1d(train_loader, val_loader, num_classes, device, writer):
    print("\n Model: R(2+1)D-18 (pretrained Kinetics-400)")
    model        = build_model(num_classes=num_classes, freeze_backbone=True).to(device)
    best_val_acc = 0.0
...
def train_slowfast(train_loader, val_loader, num_classes, device, writer):
    print("\n Model: SlowFast R50 (pretrained Kinetics-400)")
    model        = build_slowfast_model(num_classes=num_classes, freeze_backbone=True).to(device)
    best_val_acc = 0.0
    ...
    best_val_acc = run_phase(
        ...
        num_classes=num_classes, device=device, writer=writer,
        best_val_acc=best_val_acc,
        epochs=SF_PHASE1_EPOCHS, lr=SF_PHASE1_LR,
        epoch_offset=0, is_slowfast=True, ckpt_name="best_slowfast.pth"
    )

    unfreeze_slowfast(model, unfreeze_from=SF_UNFREEZE)
    best_val_acc = run_phase(
        phase=2, model=model,
        train_loader=train_loader, val_loader=val_loader,
        num_classes=num_classes, device=device, writer=writer,
        best_val_acc=best_val_acc,
        epochs=SF_PHASE2_EPOCHS, lr=SF_PHASE2_LR,
        epoch_offset=SF_PHASE1_EPOCHS, is_slowfast=True, ckpt_name="best_slowfast.pth"
    )
    return best_val_acc
...
def main():
    ...
    parser.add_argument(
        "--model",
        choices=["r2plus1d", "slowfast"],
        default="r2plus1d",
        help="Which model to train (default: r2plus1d)"
    )
    ...
    print(f"[Train] Train   : {TRAIN_DIR}")
    print(f"[Train] Val     : {VAL_DIR}")
    ...
    if args.model == "r2plus1d":
        best_acc = train_r2plus1d(train_loader, val_loader, num_classes, device, writer)
    else:
        best_acc = train_slowfast(train_loader, val_loader, num_classes, device, writer)

    writer.close()
    print(f"\n Training complete!")
    print(f"   Model        : {args.model}")
    print(f"   Best Val Acc : {best_acc:.1f}%")
    print(f"   Checkpoint   : checkpoints/best_{args.model}.pth")
